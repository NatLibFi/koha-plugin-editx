...
like( $css, qr{\.editx-folder-sources-table\s*\{[\s\S]+min-width:\s*58rem;[\s\S]+table-layout:\s*fixed;[\s\S]+width:\s*100%;}, 'CSS gives the folder source table a compact fixed layout' );
like( $css, qr{\.editx-source-success-controls\s*\{[\s\S]+grid-template-columns:\s*minmax\(0, 1fr\) minmax\(8rem, 10rem\);}, 'CSS keeps success action controls compact inside source tables' );
