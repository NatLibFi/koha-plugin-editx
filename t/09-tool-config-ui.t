...
my ($folder_sources_table) = $configure =~ m{(<table id="editx-folder-sources-table"[\s\S]*?</table>)};
...
like( $plugin_pm, qr{sub\s+_manual_selected_sources\s*\{}, 'Plugin filters staged manual checks to selected EDItX sources' );
like( $plugin_pm, qr{sub\s+_manual_stage_check_remote\s*\{}, 'Plugin can check source files as stage one' );
like( $plugin_pm, qr{sub\s+_manual_stage_resume\s*\{}, 'Plugin can resume a staged workflow after a redirect-safe GET reload' );
like( $plugin_pm, qr{sub\s+_manual_stage_download_selected\s*\{}, 'Plugin can stage selected source files into manual staging' );
...
like( $plugin_pm, qr{output_html\(\s*\$template->output\(\),\s*undef,\s*undef,\s*Koha::Plugin::Fi::KohaSuomi::Editx::FlashCookie->merge_cookies}s, 'Configure page passes flash cookies as output_html cookies, not extra options' );
...
like( $configure, qr{Active sources are scanned by the nightly plugin cron}, 'Configure page explains that active sources are scanned automatically' );
like( $configure, qr{id="editx-sftp-sources-table"\s+class="editx-config-table editx-sftp-sources-table"[\s\S]+Active[\s\S]+Source[\s\S]+Remote folder[\s\S]+After import[\s\S]+class="editx-source-details-row"[\s\S]+Local target[\s\S]+Identity file[\s\S]+id="editx-add-sftp-source"}, 'Configure page edits SFTP sources as deliberate two-line source groups' );
like( $configure, qr{class="table-responsive"[\s\S]+id="editx-folder-sources-table"\s+class="table table-striped table-sm editx-folder-source-editor"[\s\S]+Active[\s\S]+Source[\s\S]+Folder[\s\S]+Pattern[\s\S]+After import[\s\S]+Local archive[\s\S]+data-editx-folder-source-row="1"[\s\S]+id="editx-add-folder-source"}, 'Configure page edits folder sources as a responsive Bootstrap table' );
ok( defined $folder_sources_table, 'Configure page contains folder source table markup' );
unlike( $folder_sources_table // '', qr{class="[^"]*(?:editx-config-table|editx-folder-sources-table)}, 'Folder source table does not use custom table layout classes' );
unlike( $folder_sources_table // '', qr{editx-source-enabled-cell|editx-source-run-toggle|editx-source-remove-cell|editx-row-actions}, 'Folder source table does not use shared source layout classes' );
...
like( $configure, qr{placeholder="/out/editx"[\s\S]+data-editx-sftp-column="remote"[\s\S]+placeholder="\*\.xml"[\s\S]+data-editx-sftp-column="pattern"}, 'SFTP remote folder and pattern fields include concrete placeholders' );
...
like( $configure, qr{name="sftp_enabled"\s+type="hidden"[\s\S]+data-editx-enabled-toggle}, 'SFTP source enabled uses a checkbox backed by aligned hidden values' );
like( $configure, qr{name="folder_enabled"\s+type="hidden"[\s\S]+data-editx-enabled-toggle}, 'Folder source enabled uses a checkbox backed by aligned hidden values' );
like( $configure, qr{name="sftp_enabled"\s+type="hidden"\s+value="0"\s+data-editx-enabled-value\s*/>\s*<input type="checkbox" value="1"\s+data-editx-enabled-toggle aria-label="Activate this SFTP source for nightly synchronization"}, 'Initial blank SFTP source is inactive by default' );
like( $configure, qr{name="folder_enabled"\s+type="hidden"\s+value="0"\s+data-editx-enabled-value\s*/>\s*<input type="checkbox" value="1"\s+data-editx-enabled-toggle aria-label="Activate this folder source for nightly synchronization"}, 'Initial blank folder source is inactive by default' );
like( $configure, qr{input\.dataset\.editxEnabledValue !== undefined\)\s*\{[\s\S]+input\.value = "0";[\s\S]+input\.dataset\.editxEnabledToggle !== undefined\)\s*\{[\s\S]+input\.checked = false;}, 'Newly added source rows are inactive by default' );
like( $configure, qr{name="sftp_success_action"[\s\S]+value="keep"[\s\S]+value="delete"[\s\S]+value="archive"[\s\S]+name="sftp_remote_archive_dir"}, 'SFTP sources configure source cleanup after successful import' );
like( $configure, qr{name="folder_success_action"[\s\S]+value="keep"[\s\S]+value="delete"[\s\S]+value="archive"[\s\S]+name="folder_local_archive_dir"}, 'Folder sources configure local source cleanup after successful import' );
like( $configure, qr{name="sftp_pattern"\s+type="text"[\s\S]+placeholder="\*\.xml"[\s\S]+data-editx-sftp-column="pattern"}, 'SFTP filename pattern keeps a concrete placeholder' );
...
like( $configure, qr{function\s+sourceRowSelector[\s\S]+tbody\[data-editx-sftp-source-row\][\s\S]+tr\[data-editx-folder-source-row\]}, 'Configure JavaScript handles SFTP row groups and folder source rows' );
...
like( $configure, qr{function\s+validateSourceNames[\s\S]+input\.setCustomValidity\("Source name must be unique\."\)}, 'Configure JavaScript validates duplicate source names before submit' );
...
like( $css, qr{\.editx-sftp-sources-table\s*\{[\s\S]+min-width:\s*86rem;[\s\S]+table-layout:\s*fixed;[\s\S]+width:\s*100%;}, 'CSS gives the two-line SFTP source table a stable layout' );
like( $css, qr{\.editx-source-details-grid\s*\{[\s\S]+grid-template-columns:\s*5rem minmax\(10rem, 13rem\) minmax\(10rem, 13rem\) minmax\(16rem, 1fr\) minmax\(16rem, 1fr\);}, 'CSS keeps secondary SFTP source fields in one compact detail row' );
unlike( $css, qr{\.editx-folder-sources-table}, 'CSS does not keep custom folder source table sizing rules' );
like( $css, qr{\.editx-folder-source-editor th:first-child,\s*\.editx-folder-source-editor td:first-child\s*\{[\s\S]+text-align:\s*center;[\s\S]+vertical-align:\s*middle;[\s\S]+white-space:\s*nowrap;[\s\S]+width:\s*1%;}, 'CSS narrows and vertically centers the folder source Active column' );
like( $css, qr{\.editx-folder-source-editor td:first-child label\s*\{[\s\S]+display:\s*inline-flex;[\s\S]+margin:\s*0;[\s\S]+\.editx-folder-source-editor td:first-child input\[type="checkbox"\]\s*\{[\s\S]+margin:\s*0;}, 'CSS removes checkbox label spacing from the folder source Active cell' );
unlike( $css, qr{\.editx-folder-source-editor (?:input|select)|folder_local_dir"\]\s*\{[\s\S]+width:}, 'CSS does not size folder source fields yet' );
unlike( $css, qr{\.editx-config-table thead th\s*\{[\s\S]+position:\s*sticky;}, 'CSS does not make every config table header sticky' );
like( $css, qr{\.editx-productform-table thead th\s*\{[\s\S]+position:\s*sticky;}, 'CSS scopes sticky table headers to the ProductForm mapping editor' );
unlike( $css, qr{editx-sftp-remote-pattern|editx-source-success-controls|editx-folder-path-pattern}, 'CSS does not keep old nested source-cell layout helpers' );
...
like( $tool, qr{Stage 1: Check source files}, 'Tool page starts staged workflow by checking source files' );
...
like( $tool, qr{name="manual_source_id"}, 'Tool page lets staged manual runs scope the EDItX sources before checking source files' );
like( $tool, qr{Fix EDItX source configuration before using the staged workflow[\s\S]+Stage 1: Check source files}, 'Tool page keeps staged configuration warning before the disabled stage-one action' );
like( $tool, qr{name="stage_download_selected"\s+value="1"}, 'Tool page can stage selected source files' );
like( $tool, qr{Stage selected</button>\s*<a class="btn btn-default" href="\[%\s*tool_href\s*\|\s*html\s*%\]">Cancel</a>}, 'Source-file stage offers a cancel link beside the staging action' );
like( $tool, qr{name="source_file"}, 'Tool page renders source file selection checkboxes' );
like( $tool, qr{id="editx-source-files-table"[\s\S]+data-editx-stage-selection-table="source_file"[\s\S]+data-editx-check-all="source_file"}, 'Source-file stage uses a Koha DataTable-ready selection table' );
like( $tool, qr{Staged EDItX preview}, 'Tool page renders staged file preview' );
...
like( $tool, qr{id="editx-staged-files-table"[\s\S]+data-editx-stage-selection-table="stage_file"[\s\S]+data-editx-check-all="stage_file"}, 'Staged-preview stage uses a Koha DataTable-ready selection table' );
...
unlike( $tool, qr{override_duplicate_stage_files|data-editx-duplicate-override|Allow duplicate EDItX files}, 'Tool page does not offer duplicate import override controls' );
unlike( $tool, qr{editx-stage-duplicate-checkbox|data-editx-duplicate-blocked}, 'Duplicate staged file checkboxes are not marked for later enablement' );
...
unlike( $tool, qr{function\s+applyDuplicateOverride|function\s+duplicateOverrideFor}, 'Staged file selection JavaScript has no duplicate override path' );
...
like( $tool, qr{Run full intake and import}, 'Tool page starts the full-chain manual sync directly' );
...
unlike( $tool, qr{Manual download and import needs valid SFTP sources}, 'Tool page does not duplicate source warnings below the manual action form' );
...
unlike( $css, qr{accent-color:\s*#b42318|editx-stage-duplicate-selected-bg|9999px #fff1f0}, 'CSS does not keep duplicate override checkbox styling' );
...
like( $plugin_pm, qr{source_operation => "Net::SFTP::Foreign ls\(\$remote_dir\)"}, 'Manual staged workflow reports the Perl SFTP list operation' );
like( $plugin_pm, qr{sub\s+_manual_stage_list_folder_source\s*\{}, 'Manual staged workflow can list folder source files' );
like( $plugin_pm, qr{sub\s+_manual_stage_copy_folder_files\s*\{}, 'Manual staged workflow can copy folder source files into staging' );
