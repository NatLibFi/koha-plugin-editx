...
like( $configure, qr{id="editx-folder-sources-table"\s+class="editx-config-table editx-folder-sources-table"[\s\S]+Active[\s\S]+Source[\s\S]+Folder[\s\S]+Pattern[\s\S]+After import[\s\S]+Local archive[\s\S]+data-editx-folder-source-row="1"[\s\S]+id="editx-add-folder-source"}, 'Configure page edits folder sources as responsive single-row source entries' );
...
like( $css, qr{\.editx-folder-sources-table\s*\{[\s\S]+min-width:\s*54rem;[\s\S]+table-layout:\s*fixed;[\s\S]+width:\s*100%;}, 'CSS gives the folder source table a compact responsive layout' );
